...
/**
  * Created by Chongguang on 2016/12/21.
  */
object Day21 {
  ...
  def rotateBased(letter: Char): String => String = (s: String) => {
    ...
    rotateLeft(s.length - realStep)(s)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val res1 = transformations.foldLeft("abcdefgh"){
      (acc, t) => t(acc)
    }
    println(res1)

    val res2 = inversedTransformations.foldLeft("fbgdceah"){
      (acc, t) => t(acc)
    }
    println(res2)
